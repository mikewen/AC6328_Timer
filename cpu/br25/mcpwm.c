 ...
 */
 /*
 void new_timer_pwm_init(JL_TIMER_TypeDef *JL_TIMERx, u32 pwm_io, u32 fre, u32 duty)
{
    ...
    //初始化timer
    JL_TIMERx->CON = 0;
    JL_TIMERx->CON |= TIMER_SSEL_LSB;                   //时钟源选择LSB时钟源
    JL_TIMERx->CON |= TIMER_PSET_DIV_2;                 //时钟源再2分频
    JL_TIMERx->CNT = 0;                                 //清计数值
    JL_TIMERx->PRD = timer_src_clk / 2 / fre;           //设置周期
